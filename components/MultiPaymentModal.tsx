...
export default function MultiPaymentModal({
  ...
  const submitBills = useMutation(api.billShares.submitProofMany);
  const submitExpenses = useMutation(api.expenseShares.submitProofMany);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const expenseIds = items
        ...
        .map((i) => i.shareId as Id<"expenseShares">);
      const tasks: Promise<unknown>[] = [];
      if (billIds.length) tasks.push(submitBills({ shareIds: billIds, proofUrl }));
      if (expenseIds.length)
        tasks.push(submitExpenses({ shareIds: expenseIds, proofUrl }));
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Paying {items.length} item{items.length === 1 ? "" : "s"}
        </div>
        <div
        ...
        {items.map((it) => (
          ...
          >
            <div className="flex center gap-2" style={{ minWidth: 0 }}>
              <Icon name={it.kind === "bill" ? "bills" : "tag"} size={14} />
              <span
                style={{
                  whiteSpace: "nowrap",
                  overflow: "hidden",
      ...
      <div className="field">
        <label className="field-label">
          ...
          <span className="muted">
            (one screenshot covers all selected items)
          </span>
        </label>
