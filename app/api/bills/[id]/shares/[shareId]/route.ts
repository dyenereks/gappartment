...
// PATCH /api/bills/[id]/shares/[shareId] — upload proof or confirm payment
export async function PATCH(
  ...
  const share = await prisma.billShare.findUnique({
    where: { id: shareId },
    include: { bill: { include: { receiver: true } } },
  });
  ...
  if (action === "confirm_payment") {
    if (share.bill.receiverId !== userId) {
      return new NextResponse("Forbidden — only the receiver can confirm", { status: 403 });
    }
    const updated = await prisma.billShare.update({
      where: { id: shareId },
